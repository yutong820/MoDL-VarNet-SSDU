...
from utils import *
from get_instances_ssdu import *
...
def main(args):
    configs, device, epochs, start_epoch, phases, workspace, logger, writers, saver, \
        dataloaders, model, loss_f, score_fs, val_score_name, optimizer, scheduler = setup(args) # call setup function to get all the configurations
    """
    ...
    for epoch in range(start_epoch, epochs):
        for phase in phases: #['train', 'val'] or ['train']
            ...
            for i, (x, k, y, csm, trn_mask, loss_mask) in enumerate(tqdm(dataloaders[phase])):
                x, k, y, csm, trn_mask, loss_mask = x.to(device), k.to(device), y.to(device), csm.to(device), trn_mask.to(device), loss_mask.to(device)

                with torch.set_grad_enabled(phase=='train'):
                    k_loss, y_pred = model(x, csm, trn_mask, loss_mask)
                
                    # k: torch.Size([1, 2, 16, 384, 384])
                    # k_loss: torch.Size([1, 2, 16, 384, 384])
                    # y_pred: torch.Size([1, 2, 384, 384])
                    
                    loss = loss_f(k, k_loss) # k is float32, k_loss is complex64
            ...
            if args.write_image > 0 and (epoch % args.write_image == 0):
                writers[phase].add_figure('img', display_img(np.abs(r2c(x[-1].detach().cpu().numpy())), trn_mask[-1].detach().cpu().numpy(), \
                    y[-1], y_pred[-1], epoch_score[val_score_name]), epoch) # last sample info 
...
if __name__ == "__main__":
    ...
    parser.add_argument("--config", type=str, required=False, default="configs/base_varnet,k=10.yaml",
                        help="config file path")
    parser.add_argument("--workspace", type=str, default='./workspace')
    parser.add_argument("--tensorboard_dir", type=str, default='./results/modl/folder_2')
