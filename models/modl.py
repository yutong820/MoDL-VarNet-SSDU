...
#CNN denoiser ======================
# define a 2D convolve layer
def conv_block(in_channels, out_channels):
    return nn.Sequential(
        nn.Conv2d(in_channels, out_channels, 3, padding=1),# 3 is kernel size
...
class cnn_denoiser(nn.Module):
    def __init__(self, n_layers):
        super().__init__()
        layers = []
        layers += conv_block(2, 64) # first layer channel is 2(real and imaginary)
        # then map to 64 channels

        for _ in range(n_layers-2):
            layers += conv_block(64, 64)

        layers += nn.Sequential(
            nn.Conv2d(64, 2, 3, padding=1), # channel number recovers to 2 of last layer
            nn.BatchNorm2d(2)
        )
    ...
    def forward(self, x):
        idt = x # (2, nrow, ncol), input image(real and imaginary of mri data)
        dw = self.nw(x) + idt # (2, nrow, ncol)
        return dw # final denoised image
