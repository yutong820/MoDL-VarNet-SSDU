...
def setup(args):
    ...
    tensorboard_dir = os.path.join(args.tensorboard_dir, configs['config_name'])
...
def main(args):
    ...
    for i, (x, y, csm, mask) in enumerate(tqdm(dataloader)):
        ...
        with torch.no_grad():
            _, y_pred = model(x, csm, mask, mask)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--config", type=str, required=False, default="configs/base_varnet,k=1.yaml",
